...
namespace nxraport_param
{
	...
	/// </summary>
	[ComVisible(true)]
	[GuidAttribute(global_config.gc_guid_nxraport_raportparamint)]
	public class raportparamint : raportparam_
	{
		public enum eMode
		{
			MODE_ANY_VALUE = 1,
		...
		public raportparamint(repository oRepository,nxobjfield oCompareFieldInfo,
		...
		public override void build_ui_part(object oAspatorObject,int idParam)
		{
			...
			int_custom oEditLess = (int_custom)oAspator.
				e_int_customex(8,"disable='true'").element;
			oAspator.write("\t\t\t\t\t\t</p>\n");

			oAspator.write("\t\t\t\t\t\t<p keepline='true'>\n");
			oAspator.e_labelex("Wiêksze ni¿",false,"size='16'");
			int_custom oEditMore = (int_custom)oAspator.
				e_int_customex(8,"disable='true'").element;
			oAspator.write("\t\t\t\t\t\t</p>\n");

			oAspator.write("\t\t\t\t\t\t<p keepline='true'>\n");
			oAspator.e_labelex("Równe",false,"size='16'");
			int_custom oEditEq = (int_custom)oAspator.
				e_int_customex(8,"disable='true'").element;
			oAspator.write("\t\t\t\t\t\t</p>\n");

			oAspator.write("\t\t\t\t\t\t<p keepline='true'>\n");
			oAspator.e_labelex("Pomiêdzy",false,"size='16'");
			int_custom oEditRangeStart = (int_custom)	oAspator.
				e_int_customex(8,"disable='true'").element;
			oAspator.e_space(1).e_label("a").e_space(1);
			int_custom oEditRangeEnd = (int_custom) oAspator.
				e_int_customex(8,"disable='true'").element;
			oAspator.write("\t\t\t\t\t\t</p>\n");
