...
public:
	...
	virtual LPCTSTR	DecodedMessage(DWORD & rDDataLen) const;
...
LPCTSTR Base64Coder::DecodedMessage(DWORD & rDDataLen) const 
{ 
	rDDataLen = m_nDDataLen;
...
void Base64Coder::Encode(LPCTSTR szMessage)
{
	if(szMessage != NULL)
	{
		Base64Coder::Encode((const PBYTE)szMessage, sizeof(TCHAR) * _tcslen(szMessage));
	}
}
...
void Base64Coder::Decode(LPCTSTR szMessage)
{
	if(szMessage != NULL)
	{
		Base64Coder::Decode((const PBYTE)szMessage, sizeof(TCHAR) * _tcslen(szMessage));
	}
}
...
//	external class
//--------------------------------------------------------
CBase64CoDec::CBase64CoDec(LPCVOID pBinary,long nBinLen)
{
	m_poImplementation = new Base64Coder();
	m_poImplementation->Encode((const PBYTE) pBinary,(DWORD) nBinLen);
}

CBase64CoDec::CBase64CoDec(LPCTSTR lpBase64)
{
	m_poImplementation = new Base64Coder();
	m_poImplementation->Decode(lpBase64);
}
...
LPCTSTR CBase64CoDec::Encoded() const
{
	ASSERT(m_poImplementation);
	return m_poImplementation->EncodedMessage();
}

LPCVOID CBase64CoDec::Decoded(DWORD & rnLen) const
{
	ASSERT(m_poImplementation);
	return (LPCVOID) m_poImplementation->DecodedMessage(rnLen);
}
